...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save']) || !empty($_COOKIE['update'])) {
    ...
    $messages['success'] = '<div style="background-color: green; color: white; padding: 10px 20px; border-radius: 40px;">';
    ...
    if (!empty($_COOKIE['update'])) {
      $messages['success'] .= 'Кликните в любом месте, чтобы продолжить';
      // $messages['success'] .= '<div><form action="logout.php" method="POST"><button class="border-radius margin-bottom" onclick="">Выйти</button></form></div>';
    }
  ...
  if (session_start() && !empty($_SESSION['login'])) {
    ...
    $db_user = "u20335"; // Логин БД
    $db_password = "2684745"; // Пароль БД
    
    // Подключаемся к БД
    $db = new PDO('mysql:host=localhost;dbname=u20335', $db_user, $db_password,
  ...
  // иначе берём данные из куки
    ...
    $values['datebirth'] = empty($_COOKIE['datebirth_value']) ? '' : strip_tags($_COOKIE['datebirth_value']);
    ...
    if(!empty($_COOKIE['superskills_value'])) {
      $ssCoockie = json_decode(strip_tags($_COOKIE['superskills_value']));
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их.
  ...
  // кроме логина и пароля.
    $user = 'u20335';
    $password = '2684745';
    $db = new PDO('mysql:host=localhost;dbname=u20335', $user, $password,
        array(PDO::ATTR_PERSISTENT => true));
